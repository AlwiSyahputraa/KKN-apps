...
@section('content')
    ...
    <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control @error('konten') is-invalid @enderror" id="konten" name="konten" rows="5" required>{{ old('konten', $berita->konten) }}</textarea>
            @error('konten')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            <label for="url_gambar" class="form-label">Ganti Gambar (Opsional)</label>
            ...
            @if($berita->url_gambar)
                <div class="mt-2">
                    ...
                    <img src="{{ asset($berita->url_gambar) }}" alt="{{ $berita->judul }}" width="150" class="img-thumbnail">
                </div>
            @endif
        </div>
